...
contract EOAInheritanceViaEIP7702Test is Test {
    ...
    function setUp() public {
        // Deploy contracts for unit testing
    ...
    function testCompleteEOAInheritanceFlow() public {
        ...
        // Generate test block header and account state proof using test helper
        ...
        bytes memory transferCall = abi.encodeWithSelector(token.transfer.selector, recipient, 100 * 10 ** 18);
        vm.prank(inheritor);
    ...
    function testEOABalanceQueriesDirect() public {
        ...
        vm.etch(eoaAccount, address(controller).code);
        vm.store(eoaAccount, bytes32(uint256(0)), bytes32(uint256(uint160(address(inheritanceManager)))));
        ...
        bytes memory tokenTransferCall = abi.encodeWithSelector(token.transfer.selector, recipient, 300 * 10 ** 18);
        ...
        assertEq(
            ...
            initialRecipientTokens + 300 * 10 ** 18,
    ...
    function _setupAndClaimInheritance() internal {
        ...
        uint256 inactivityBlock = currentBlock + 100;

        // Create test block header using test helper
        ...
        // Use consistent nonce value for testing
        ...
        StateVerifier.AccountStateProof memory accountStateProof = StateVerifier.AccountStateProof({
            ...
            storageHash: keccak256(abi.encodePacked("storage", eoaAccount)),
            codeHash: keccak256(abi.encodePacked("code", eoaAccount)),
            proof: stateProofHelper.generateAccountProof(eoaAccount, testNonce, testBalance)
        });
